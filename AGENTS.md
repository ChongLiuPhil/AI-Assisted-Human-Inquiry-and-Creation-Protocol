...
When this repository is being used to configure, compose, provision, upgrade, publish, or operate a downstream project together with PPF, Vault Interface, or the Starter, first read `docs/ECOSYSTEM.md`, `ecosystem.yaml`, and the [canonical cross-repository Agent Retrieval Contract](https://github.com/ChongLiuPhil/Inquiry-Publishing-Project-Starter/blob/main/docs/AGENT_RETRIEVAL_CONTRACT.md). For a new project, also read the [Project Provisioning Contract](https://github.com/ChongLiuPhil/Inquiry-Publishing-Project-Starter/blob/main/docs/PROJECT_PROVISIONING_CONTRACT.md). Reconstruct all four public component roles before making cross-component decisions.
...
For original or unpublished downstream work, the default full-stack posture is full AHICP + full PPF + Vault Interface, private canonical source, and restricted/authenticated Continuous Web until explicit human public-release authorization. For ordinary private downstream projects, prefer `workers-builds-native` + `private-project-quota-saver` under the personal `ChongLiuPhil` account and allow one short human-assisted GitHub → Cloudflare bootstrap per project. Content-only changes must not start GitHub Actions; configuration PRs use the thin contract gate, heavy GitHub workflows are manual, and Cloudflare Workers Builds owns the automatic main-branch Web build. After the connection is verified, require a second content-only push to auto-deploy without renewed authorization or a duplicate Actions production build. Public framework repositories retain full CI. Public links never grant private-state access. Provider credential plaintext must never enter chat/model context; the Trusted Secret Broker applies only to the optional advanced external-CI profile. Do not enable paid Actions usage without explicit human authorization. Human handoffs must provide numbered operator-level steps, secret boundaries, completion evidence, verification, and rollback.
