...
For original or unpublished downstream work, the default full-stack posture is full AHICP + full PPF + Vault Interface, private canonical source, and restricted/authenticated Continuous Web until explicit human public-release authorization. For ordinary new projects, prefer `workers-builds-native` under the personal `ChongLiuPhil` account and allow one short human-assisted GitHub → Cloudflare bootstrap per project. After that connection is verified, require a second push to auto-deploy without renewed authorization. Public links never grant private-state access. Provider credential plaintext must never enter chat/model context; the Trusted Secret Broker applies only to the optional advanced external-CI profile. Human handoffs must provide numbered operator-level steps, secret boundaries, completion evidence, verification, and rollback.
