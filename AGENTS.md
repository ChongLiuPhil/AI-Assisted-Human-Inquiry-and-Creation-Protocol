...
For original or unpublished downstream work, the default full-stack posture is full AHICP + full PPF + Vault Interface, private canonical source, and restricted/authenticated Continuous Web until explicit human public-release authorization. After verified platform bootstrap, prefer `agent-provisioned-external-ci` and reuse bounded standing authorization for ordinary private/restricted setup inside the approved scope instead of repeatedly escalating routine provider work. Public links never grant private-state access. Project deployment-token plaintext must remain inside the trusted Secret Broker. Cloudflare human handoffs remain necessary for genuinely reserved boundaries and must provide numbered operator-level steps, secret boundaries, completion evidence, verification, and rollback.
