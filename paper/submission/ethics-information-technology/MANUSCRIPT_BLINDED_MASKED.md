...
“Project Memory” is not a term introduced by the protocol. Broader organizational-memory research had already theorized memory at the organizational level in terms of information acquisition, retention, and retrieval, while explicitly addressing the risk of anthropomorphism (Walsh & Ungson, 1991).
...
### 2.3 Boundary between Agent memory and the protocol's Project Memory

Despite the proximity, **Agent memory and the protocol's Project Memory are not the same analytical layer.**
...
Critical project state should not be bound to one model or platform. Models may change while the project continues. A version-controlled repository is the protocol's present reference implementation rather than a theoretical prerequisite; the same architecture could be implemented on another durable substrate with suitable versioning, access control, query, write-back, and migration capabilities.
...
Table 1 summarizes the functional memory roles used in the protocol.

**Table 1. Functional memory roles used in the protocol.**
...
When AI encounters multiple plausible interpretations of a high-impact issue, the protocol requires the uncertainty to be externalized rather than silently resolved. Examples include:
...
The protocol is not intended to replace prior project-memory or Agent-memory research. More accurately, it sits at the intersection of three research traditions. Table 2 compares the resulting emphases across these approaches:

**Table 2. Comparison of organizational/project memory, Agent memory, and Proposed governed Project Memory.**
...
Generative AI tools were used during development of this manuscript for iterative drafting, structural reorganization, literature organization, bilingual synchronization, consistency checking, and repository-oriented implementation support. AI-generated outputs were treated as proposals rather than as autonomous scholarly decisions. The human author(s) defined and approved the research direction, the core conceptual framework, substantive claim boundaries, major governance decisions, and the final use of sources and arguments, and remain responsible for citation verification and the final manuscript. No AI system is listed as an author. This disclosure describes substantive generative use and is not limited to AI-assisted copy editing.

## Data availability statement

No original empirical dataset was generated or analyzed for this conceptual and methodological article. The manuscript proposes a future evaluation program but reports no completed empirical experiment or dataset-based result. Accordingly, there are no research data supporting empirical results in the present manuscript.
