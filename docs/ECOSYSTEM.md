...
The default new-project baseline is **full AHICP + full PPF + Vault Interface**, with Vault Interface used only for the public metadata contract. Reduced profiles require explicit human selection. For ordinary **private downstream projects**, the preferred infrastructure profile is `workers-builds-native` + `private-project-quota-saver`: keep the source repository private under the personal `ChongLiuPhil` account, allow one short human-assisted GitHub → Cloudflare project bootstrap, protect the Worker with Access, and verify that a second content-only push deploys automatically without renewed authorization or a duplicate GitHub Actions production Web build. Content-only changes do not start GitHub Actions, configuration PRs use one lightweight contract gate, heavy GitHub workflows are manual, and Cloudflare Workers Builds owns the automatic main-branch Web build. Public framework repositories retain their full CI. `agent-provisioned-external-ci` remains an optional advanced profile. Public release, source-repository publication, reader expansion, domains/DNS, provider-scope expansion, paid-plan changes, and enabling paid Actions remain human-reserved. A project records its actual adoption and pinned revisions instead of copying a second authoritative framework.
...
Cloudflare deployment and reader-access operations must follow the [Starter Project Provisioning Contract](https://github.com/ChongLiuPhil/Inquiry-Publishing-Project-Starter/blob/main/docs/PROJECT_PROVISIONING_CONTRACT.md) and the [PPF/Starter operational guide](https://github.com/ChongLiuPhil/Inquiry-Publishing-Project-Starter/blob/main/docs/CONTINUOUS_WEB_CLOUDFLARE.md). The default path allows the account holder to perform the repository-level Cloudflare Git connection and Worker Access setup once for each project; the Agent should continue every other safe machine-operable step and then verify a second push without renewed authorization. Provider credential plaintext must never enter chat/model context. The Trusted Secret Broker belongs only to the optional advanced external-CI profile.
