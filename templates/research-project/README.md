...
10. Build `docs/argument-map.zh-CN.md` and its English mirror as an AI-maintained Layer 2 working representation. AI may help propose, organize, and express the framework, but do not characterize AI as a cognitive subject or say that it “performs cognitive labor / cognitive tasks.” AI may share work, but humans must remain the bearers of responsibility.
11. Initialize `docs/framework-status.zh-CN.md` as `WORKING-FRAMEWORK` with no approved snapshot unless the human has explicitly approved one. Framework Approval requires the human to clearly understand, carefully review, and confirm every substantive element actually represented in the proposed framework; if the artifact enters public circulation, Final Artifact Approval must also preserve identifiable human bearers of responsibility.
